...
export function registerQuestionHandlers() {
  ...
  ipcMain.on('save-question', (event, payload: Question) => {
  ...
  ipcMain.handle('get-exam', async (event, { id, subject }) => {
    ...
    if (!examData || !students) return event.sender.send('get-exam-response', {success: false, message: '문항 및 학생 파일 로드 중 오류 발생'})
