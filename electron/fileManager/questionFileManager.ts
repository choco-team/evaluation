...
export function getQuestionFilePath(subject: string): string {
  return path.join(QUESTION_DIR, `${subject}.json`);
}

export function loadQuestions(subject: string | null): Question[] {
  ensureQuestionDir();
...
export function saveQuestions(subject: string, newQuestion: Question): void {
  ensureQuestionDir();
  const filePath = getQuestionFilePath(subject);
...
export function deleteQuestionById(subject: string, id: string): Question[] {
...
export function saveQuestionList(subject: string, questions: Question[]): void {
  ensureQuestionDir();
  const filePath = getQuestionFilePath(subject);
  fs.writeFileSync(filePath, JSON.stringify(questions, null, 2), 'utf-8');
}
