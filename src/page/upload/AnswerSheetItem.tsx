...
import { 
  QuestionFormat, 
  CorrectAnswerItem 
} from '../../common/types/answer-sheet-types';
...
export function AnswerSheetItem({ index, id }: Props) {
  ...
  const getCurrentAnswer = () => {
    // 선택형 문제의 경우 숫자 또는 문자열 처리
    if (item.format === 'select') {
      // 숫자인 경우 그대로 반환, 아니면 빈 문자열
      return typeof correctAnswer[index] === 'number' 
        ? correctAnswer[index] 
        : '';
    }
    // 다른 형식의 문제는 문자열로 처리
  ...
  const handleOptionCountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const currentAnswer = correctAnswer[index];
  ...
  // 정답 변경 핸들러
  const handleCorrectAnswerChange = (value: CorrectAnswerItem) => {
