...
export function useSubjectManager() {
  ...
  const newSubjectExists = subjects.includes('새 교과');
    ...
    try{
     ...
     if (!result) {
       await invoke('show-error-dialog', '평가를 위한 프롬프트 생성 중 오류가 발생하였습니다');
       return;
     }
      await invoke('show-success-dialog', '프롬프트 생성에 성공했습니다 프롬프트 파일 저장경로로 이동합니다')
     const success = await invoke(`open-folder`, null);
     if (!success){
      await invoke('show-error-dialog', '해당 위치를 찾을 수 없습니다')
     }
    return result
    }
    catch (error) {
      await invoke('show-error-dialog', String(error))
    }
    finally {
      setIsLoading(false)
    }
