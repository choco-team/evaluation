...
export function useSubjectManager() {
  ...
  const newSubjectExists = subjects.includes('새 교과');
    ...
    try{
     ...
     if (!result) window.electronAPI.showMessageBox('평가 기준 프롬프트 생성 중 오류가 발생하였습니다')
      window.electronAPI.showMessageBox('평가 기준 프롬프트 생성에 성공했습니다 해당 파일 저장경로로 이동합니다')
     const success = await invoke(`open-folder`, null);
     if (!success){
      window.electronAPI.showMessageBox('해당 위치를 찾을 수 없습니다')
     }
    return result
    }
    catch (error) {
      window.electronAPI.showMessageBox(error)
    }
    finally {
      setIsLoading(false)
    }
